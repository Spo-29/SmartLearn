...
const LessonSort = ({ show, chapter, loading, onClose, onSave }) => {
  const [items, setItems] = useState(chapter?.lessons || []);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!chapter?.id || items.length < 2) {
      onClose();
      return;
    }

    await onSave(chapter.id, items);
  };
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <Modal.Body>
          <p className="small text-muted">{chapter?.title || 'Chapter'}</p>
          {items.length < 2 ? (
            <p className="text-muted mb-0">Add at least 2 lessons to reorder.</p>
          ) : (
            <DragDropContext onDragEnd={handleDragEnd}>
              <Droppable droppableId="lessons-sort-list">
                {(provided) => (
